...
%Tweaked for speed (factor 3) 2010, Piotr Majdak, piotr@majdak.com
...
if flag == 1
	abuff = ones(1,n,'double');
else
	rand('state',sum(100*clock));
	
	while 1
		abuff = double(round(rand(1,n)));
...
y=zeros(2^n-1,1,'double');

if taps==4
  for i = (2^n)-1:-1:1  
    y(i) = xor (xor(abuff(tap1),abuff(tap2)), ...
                  xor(abuff(tap3),abuff(tap4)));
    abuff = [y(i) abuff(1:n-1)];
  end
else
  for i = (2^n)-1:-1:1
    y(i) = xor(abuff(tap1),abuff(tap2));		%feedback bit
    abuff = [y(i) abuff(1:n-1)];
  end
end
